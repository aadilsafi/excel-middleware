...
class ProcessOrderFilesSecondary extends Command
{
    protected $signature = 'order-files-process-secondary';
    ...
    public function handle()
    {
        try {
            ...
            Log::info('processing ftp for RSR (67883) files for orders command');
            $sellerCloudService = new \App\Services\SellerCloudService();

            // Define the FTP directory
            $directory = 'eo/outgoing';
            $files = Storage::disk('secondary_rsr')->files($directory);
            ...
            foreach ($files as $file) {
                ...
                if (strpos($filePath, 'ECONF') !== false) {
                    ...
                    if ($tracking_number && $invoice_number) {
                        ...
                        Log::info('RSR 67883 Tracking : ' . $tracking_number);
                        ...
                        if(!$res){
                            Log::error('RSR 67883 Failed to update order id: ' . $order_id . ' and tracking number: ' . $tracking_number. ' at ' . $ship_date);
                            continue;
                        }
                    } else {
